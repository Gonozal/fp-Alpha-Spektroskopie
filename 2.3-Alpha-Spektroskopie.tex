...
Wie oben erwähnt, wird die prominente Linie (bei 5,486 MeV) des $\alpha$-Spektrums als bekannt vorausgesetzt und die Amplitude es Pulsgenerators auf diese Energie eingestellt. Die anderen Kalibrationspunkte werden bei Abschwächung um den Faktor 2 und 5 aufgenommen. Im ersten Schritt werden nun die Messereignisse des Pulsers über den Kanalnummern aufgetragen und gefittet. Die Breite der Pulserlinien ist daher von Belang, da sie den Anteil des elektronischen Rauschens anzeigt. Dadurch, dass die Energie von 5,486 MeV bekannt ist, kann nun durch diese 4 Messungen eine Kalibrationsgerade berechnet werden, die in der zweiten Grafik dargestellt ist. Da bei Reduzierung auf $\frac{1}{10}$ der Amplitude der Fehler sinnvollerweise bezogen auf $\frac{9}{10}$ des Wertes berechnet wird, ist es natürlich, dass der Fehler für kleinere Energien zunimmt, wie im Graphen zu sehen ist. Zudem sind Steigung und Achsenabschnitt korreliert, wodurch keine Gaußsche Fehlerfortpflanzung angewendet werden kann; der Fehler ergibt sich vielmehr mit der Korrelationsmatrix C zu:
...
Im Folgenden betrachten wir das $\alpha$-Spektrum von $^{241}Am$ inkl. Hintergrundstrahlung. Zu erwarten wären 3 überlagerte Exponentialfunktionen aufgrund der Hintergrundstrahlung, was wir mittels eines Fits in logarithmischer Darstellung bestätigen können, hier erhalten wir 3 Geraden. Zu sehen sind außerdem die Gauß-Fits für die drei erwarteten $\alpha$-Peaks und einen vierten Peak bei höherer Energie. Es wurde mit folgenden Fit-Parametern gearbeitet: 
...
\begin{center}
  \begin{tabular}{|p{4cm}|p{5cm}|}
    ...
    ln(h) & G in 1/keV \\ \hline
      & 1,817$\cdot 10^{-4}$  \\ \hline
       & 1,831$\cdot 10^{-4}$  \\ \hline
      & 1,851$\cdot 10^{-4}$ \\ \hline
        \end{tabular}
\end{center}
